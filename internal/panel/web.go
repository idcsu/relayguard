...
uninstall_panel(){
  ...
  systemctl daemon-reload
  read -rp "是否删除数据目录 ${DATA_DIR}？输入 YES 确认: " confirm
...
restore_panel(){
  ...
  read -rp "请输入备份文件完整路径: " backup_file
  [ -f "$backup_file" ] || { red "备份文件不存在"; return; }
  read -rp "恢复会覆盖当前数据，脚本会先自动备份当前数据。输入 确认恢复 继续: " confirm
...
install_agent(){
  need_root
  install_deps
  [ -n "$PANEL" ] || read -rp "请输入面板地址，例如 https://panel.example.com: " PANEL
  [ -n "$TOKEN" ] || read -rp "请输入节点注册 Token: " TOKEN
...
uninstall_agent(){
  ...
  if [ -x "${BIN_DIR}/${APP}" ]; then
    read -rp "是否清理 RelayGuard 创建的防火墙托管规则？输入 YES 确认: " fw_confirm
    [ "$fw_confirm" = "YES" ] && "${BIN_DIR}/${APP}" firewall rescue || true
  fi
  rm -f /etc/systemd/system/${SERVICE}
  systemctl daemon-reload
  rm -f "${BIN_DIR}/${APP}"
  read -rp "是否删除 Agent 数据目录 ${DATA_DIR}？输入 YES 确认: " confirm
  [ "$confirm" = "YES" ] && rm -rf "$DATA_DIR"
  green "卸载完成"
}
