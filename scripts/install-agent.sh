...
install_deps(){
  if command -v apt-get >/dev/null 2>&1; then
    ...
    dnf install -y curl ca-certificates iptables iproute
  elif command -v yum >/dev/null 2>&1; then
    yum install -y curl ca-certificates iptables iproute
  elif command -v apk >/dev/null 2>&1; then
    apk add --no-cache curl ca-certificates iptables iproute2
  else
...
ExecStart=${BIN_DIR}/${APP} -panel ${PANEL} -token ${TOKEN} -data ${DATA_DIR} -ssh-ports ${SSH_PORTS} -allow-tcp ${ALLOW_TCP} -allow-udp ${ALLOW_UDP}
Restart=always
RestartSec=3
LimitNOFILE=1048576
...
SERVICE_EOF
  ...
  green "RelayGuard Agent 安装完成"
  yellow "查看状态：systemctl status ${SERVICE} --no-pager"
  yellow "查看日志：journalctl -u ${SERVICE} -f"
}
...
menu(){
  ...
  case "$n" in
    ...
    7) need_root; "${BIN_DIR}/${APP}" firewall rescue;;
    0) exit 0;;
    *) red "无效选项";;
  esac
}
